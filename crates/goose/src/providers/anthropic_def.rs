...
use goose_providers::{
    ...
    formats::anthropic::{AnthropicFormatOptions, ANTHROPIC_PROVIDER_NAME},
    model::ModelConfig,
};
...
async fn from_env(
    ...
    Ok(AnthropicProviderBuilder::new(api_client, model).build())
}
...
pub fn from_custom_config(
    ...
    Ok(AnthropicProviderBuilder::new(api_client, model)
...
mod tests {
    ...
    fn make_provider_with_server(
        ...
        let api_client = ApiClient::new_with_tls(server_uri.to_string(), auth, None)
            ...
            .with_header("anthropic-version", ANTHROPIC_API_VERSION)
            .unwrap();
        AnthropicProviderBuilder::new(api_client, ModelConfig::new_or_fail("claude-test"))
    ...
    fn from_custom_config_rejects_static_only_without_models() {
        ...
        let err = from_custom_config(ModelConfig::new_or_fail("claude-test"), config, None)
            .err()
